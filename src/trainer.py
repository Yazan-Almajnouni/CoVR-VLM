...
def one_epoch(phase, model, loader, device, optimizer=None):
    ...
    model.head.train(training) # set head only
    total_loss = 0.0
    total_samples = 0

    # instantiate HN‐NCE once per epoch
    criterion = HardNegativeNCE(alpha=1.0, beta=0.5)
    temp = 0.07
    dtype = model.dtype
    ...
    with torch.set_grad_enabled(training):
        for batch in loader:
            ...
            inf_start = time.time()
            query_emb = model(inputs).to(device)     # (B, D)
            inf_end = time.time()
            print(f"inference time = {(inf_end - inf_start):.2f}s")

            target_emb = target_emb.to(device=device, dtype=dtype)
...
def train(model,
    ...
    for epoch in range(epochs):
        ...
        if current_r1 >= best_r1:
            ...
            best_state = {
                'epoch':           epoch,
                'model_state':     model.head.state_dict(), # only save head
                'optimizer_state': optimizer.state_dict(),
                'train_loss':      train_loss,
                # 'val_loss':        val_loss,
                'recall':          rec,
            }
            filename = f"{model.head_name}checkpoint_best_r1={best_r1:.4f}_epoch={epoch}.pt"
